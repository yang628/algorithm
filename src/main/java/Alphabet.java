...
public class Alphabet {
    ...
    public Alphabet(String alpha) {
        ...
        // check that alphabet contains no duplicate chars 主要检查改定义的字表中，是否有重复的。有则报错，默认全都是false。
        ...
        for (int i = 0; i < alpha.length(); i++) {
            ...
            //在对应的字符 C（对应的字符的十进制）位置 传入true。  字符的ASIIC码对应数组的索引
            unicode[c] = true;
        }
        //字符传转成字符数组
        ...
        // 由于R可以大到65,536，所以不能使用char
        for (int c = 0; c < R; c++){
            //每一个字符数组 所对应的元素的[ASSIC码]做为inverse的索引。
